# DecisionLab — UI/UX DATA SHEET V2.3.2
Version: 2.3.2 — Compact DecisionLab Brand Lock-up Refinement
Status: CONSOLIDATED SOURCE OF TRUTH — V2.3.2 BRAND AMENDMENT + V2.3.1 WORKSPACE ARCHITECTURE + LOCKED V2.2.15 TASK MECHANICS
Purpose: Preserve the V2.3.1 Client Advisory Workspace architecture while refining the sidebar DECISIONLAB brand lock-up.

## V2.3.2 AUTHORITATIVE BRAND OVERRIDE

Where the V2.3.1 specification requires a `Client Advisory Workspace` secondary line beneath the sidebar DECISIONLAB logo, **V2.3.2 supersedes that instruction**.

The sidebar lock-up is now `[D mark] DECISIONLAB` only. The mark is 10° clockwise, compact relative to the wordmark, and contains an upright, optically centred `D` using an Apple-oriented system sans-serif stack. `Client Advisory Workspace` remains the global workspace label in the persistent topbar and elsewhere, continuing to replace `Independent Preparation Workspace` across the application.

Reference scale at the current desktop sidebar: 22px mark, 18px wordmark (approximately 2px visual extension above and below the wordmark line box). The mark should extend only slightly above and below the wordmark rather than functioning as a dominant badge.

No task mechanics, data, scoring, routing, persistence, timers, result logic, Sea Wolf stage behaviour, or Learning/Simulation behaviour change in V2.3.2.

---

## V2.3.1 WORKSPACE ARCHITECTURE — RETAINED EXCEPT WHERE SUPERSEDED ABOVE

## V2.3.1 AUTHORITATIVE UX/UI ARCHITECTURE LOCK

Where any earlier branding, workspace-label, Command Centre, Sea Wolf white stage-rail, Next Site Insight visibility, or Treatment average-summary instruction conflicts with this section, **V2.3.1 wins**.

### Product and workspace identity
- learner-facing product: **DECISIONLAB**;
- persistent environment label: **Client Advisory Workspace**;
- `Client Advisory Workspace` replaces `Independent Preparation Workspace` everywhere in the running application;
- old `Solve It / READINESS LAB` learner-facing branding is removed from the application shell;
- historical/reference material may retain source terminology when necessary for provenance.

Approved sidebar lock-up:
- native lime `D` mark;
- `DECISION` stronger/white;
- `LAB` lighter/lime;
- sidebar secondary line removed in V2.3.2; `Client Advisory Workspace` remains the persistent workspace label in the topbar and other workspace contexts.

### Dark sidebar structure
The top-level desktop sidebar is exactly:

`Home`
`01 Red Rock Study`
`02 Sea Wolf Study`

- `Command centre` is removed;
- no standalone `Results` item is exposed;
- no separate `Learning Path` heading is required between Home and the workstreams;
- Home is the engagement-entry destination.

Red Rock continues to expand its contextual Investigation / Analysis / Report subnavigation beneath `01 Red Rock Study`.

Sea Wolf now expands its current-site stages beneath `02 Sea Wolf Study`:
- Site Brief;
- Characteristics;
- Categorisation;
- Initial Pool;
...
`Transfer Review` may appear only when that transient stage is active. Sea Wolf stage labels remain progress/context indicators and must not allow free navigation around locked sequencing.

### Sea Wolf workspace redesign
Remove the separate white Task 2 stage rail. Desktop Sea Wolf becomes:

`dark application sidebar | expanded main work canvas | Site Information`

The recovered width belongs to the main interaction canvas. A compact case/site context line may appear above the task card and may also hold the active timer.

### Next Site Insight
`Next Site Insight` appears **only during Categorisation**. It is hidden during Site Brief, Transfer Review, Characteristics, Initial Pool, Prospect Selection, Treatment, Site Result and Task 2 Result. This is a presentation rule only; the underlying routing data and logic remain unchanged.

### Treatment right-panel simplification
Remove learner-facing:
- `Selected trio`;
- `Avg P`;
- `Avg M`;
- `Avg E`.

The internal treatment scoring model remains locked and may continue to use the existing arithmetic logic. Learner-facing Treatment guidance should instead focus on whether the selected three-microbe portfolio collectively fits the site's Permeability, Mobility, Energy and trait requirements.

### Home and topbar hierarchy
- topbar workspace label: `Client Advisory Workspace`;
- on Home, engagement-specific naming is carried inside the engagement hero/content rather than used as the application identity;
- Fairhaven Environmental Trust / Environmental Recovery Programme remains the current engagement context.

### Results
Preserve the V2.2.15 embedded-result architecture:
- Red Rock ends in Task 1 Result / Review;
- Sea Wolf ends in Task 2 Result / Review;
- results remain attached to their workstreams;
- result exit wording is `Return Home` rather than `Return to Command Centre`.

### Mechanics and scoring preserved
V2.3.1 does not change Red Rock or Sea Wolf datasets, answer keys, fixed decks, routing logic, transfer rules, prospect logic, treatment-effectiveness calculation, scoring weights, timers, persistence, session progression, or Learning/Simulation behaviour.

### V2.3.1 acceptance criteria
1. DECISIONLAB is the visible product identity.
2. Client Advisory Workspace is the global workspace label.
3. Sidebar is Home → 01 Red Rock Study → 02 Sea Wolf Study.
4. Command centre is absent.
5. Sea Wolf has no white stage rail.
6. Sea Wolf stage context appears beneath the active study in the dark sidebar.
7. Sea Wolf's main work canvas expands into the removed rail width.
8. Next Site Insight renders only for Categorisation.
9. Treatment has no Selected Trio averages block.
10. Embedded Task 1 / Task 2 result behaviour remains intact.
11. Locked game mechanics/scoring remain unchanged.

---

# INHERITED V2.2.15 CONSOLIDATED BASELINE

The complete V2.2.15 source-of-truth follows. It remains authoritative for all areas not superseded by the V2.3.1 lock above.

# Solve It Readiness Lab — UI/UX DATA SHEET V2.2.15
Version: 2.2.15 — Embedded Task Results, Navigation Simplification, Streamlined Sea Wolf Review, Locked Red Rock / Sea Wolf Mechanics
...
- Do not show a separate `Reference maximum feasible effectiveness` note beneath a Sea Wolf site-result card; site-level feasibility is reviewed later in the streamlined Treatment Construction review.
...
# 11.11 Task 2 Result — streamlined client close-out

The Task 2 Result is a client-led workstream close-out rather than a detached scorecard or a full decision-log dump.

Locked sequence:

**Sea Wolf workstream complete. Review feedback below. → Dr. Reyes Client Debrief → Primary Concern + weighted Performance Snapshot → Detailed review · Site Profiling → Detailed review · Treatment Construction → Readiness Confidence → Return Home.**

### Results hero
- Keep the engagement-review eyebrow, readiness band and overall score.
- Remove `Selected attempt: ...`.
- Remove the learner-facing simulator disclaimer from the hero.
- In a full simulation, any separate full-simulation benchmark context may remain only where already specified; it must not recreate selected-attempt or disclaimer copy.

### Workstream completion strip
- Keep `Workstream 02 · Dr. Paula Reyes`.
- Keep `Sea Wolf workstream complete. Review feedback below.` and the Task 2 score.
- Do not repeat the selected Sea Wolf study title in this strip.

### Dr. Reyes Client Debrief
The client debrief appears first and uses a predefined **strong / mid-range / weak** response based on overall Task 2 performance. It should be 2–3 short paragraphs and must not read component percentages aloud. It answers:
...
### Primary Concern + weighted Performance Snapshot
...
It must identify the most material client-facing decision issue and link earlier decisions to later treatment consequences instead of mechanically naming the lowest percentage.

The compact Performance Snapshot remains inside the same Primary Concern card, but it displays **earned weighted points out of the available component points**, not the raw component percentage. Example:
- Site Profiling: `6.7 / 10`;
- Categorisation & Transfer: `17.7 / 30`;
- Prospect Selection: `16.8 / 25`;
- Treatment Construction: `29.8 / 35`.

The four displayed values must reconcile visibly to the overall Task 2 score, subject only to normal display rounding. There is no separate Assessment Benchmark card.

### Streamlined detailed review
Use only **Correct** (green) and **Needs review** (red) as learner-facing statuses.

Show only:
1. **Detailed review · Site Profiling** — selected characteristic pair, reference lens and status for each site. Do not repeat generic acceptance/scoring-rule prose beneath every site.
2. **Detailed review · Treatment Construction** — submitted trio, achieved effectiveness, maximum feasible effectiveness and status for each site.

Do **not** show separate Detailed Review sections for:
- Categorisation & Transfer;
- Prospect Selection.

Those stages remain fully scored and continue to inform the Task 2 score, Dr. Reyes Client Debrief and Primary Concern, but their card-by-card/round-by-round decision logs are not learner-facing on the Result page.

Within Treatment Construction review, do not show:
- `Performance against feasible maximum`;
- trio-average calculation strings;
- repeated target ranges;
- desired/excluded trait calculation strings;
- generic constrained-site explanatory paragraphs.

A constrained site's **maximum feasible effectiveness** remains visible, and achieving that maximum still earns **Correct** and full site credit.

### Site-level Treatment Result
After a treatment is locked in Learning Mode:
- keep the site-result percentage and a concise Dr. Reyes reaction inside the accent result card;
- remove the separate `Reference maximum feasible effectiveness for this site` line beneath the card;
- do not duplicate scoring-engine calculations in the client statement.

### Remove generic post-review diagnostics
The final Results page does not show the legacy:
- Reasoning Profile;
- Primary Performance Concern;
- Recommended Next Practice.

The integrated client debrief and Primary Concern already perform the useful diagnostic function.

# 11.12 Readiness confidence and closing action

Keep a compact **Readiness Confidence** section. Its level may continue to use valid attempt-history logic where available.

Do not show generic instructional copy such as `Complete multiple full simulations to establish consistency.`

Closing action:
- **Return Home** only.
- Remove `Try Another Simulation` from the Result-page closing actions.

# 11.13 Result-screen hierarchy

For a Sea Wolf result, the learner-facing hierarchy is:
1. **Practice Result / Overall Practice Readiness hero**;
2. **Workstream 02 completion strip**;
3. **Dr. Reyes Client Debrief**;
4. **Primary Concern + weighted Performance Snapshot**;
5. **Detailed review · Site Profiling**;
6. **Detailed review · Treatment Construction**;
7. **Readiness Confidence**;
8. **Return Home**.

No separate Decision Review, Assessment Benchmark, Reasoning Profile, Primary Performance Concern, Recommended Next Practice, Categorisation decision log or Prospect Selection decision log is exposed in this flow.
...
### Task 2
The learner-facing integrated review is intentionally selective. Show only:
- Site Profiling: selected two-characteristic lens, reference lens and Correct / Needs review;
- Treatment Construction: submitted treatment, achieved effectiveness, maximum feasible effectiveness and Correct / Needs review.

Categorisation & Transfer and Prospect Selection remain scored internally and feed the client synthesis, but their card-by-card and round-by-round logs are not rendered in the Result review. Detailed scoring-engine calculations remain internal.
...
Below:
- client debrief;
- Primary Concern with an integrated compact performance snapshot;
- streamlined detailed review appropriate to the completed workstream;
- Readiness Confidence;
- Return Home.

For Sea Wolf, the learner-facing detailed review is limited to Site Profiling and Treatment Construction. Do not include the legacy Reasoning Profile, Primary Performance Concern, Recommended Next Practice, separate Assessment Benchmark or Decision Review action.
...
- **Salvanova Exhibit 2:** draggable with-wolf vegetation/tourism projection table + separate vegetation and tourism trend visuals; the no-intervention Year-5 vegetation assumption belongs in Study Information and is not duplicated in the exhibit.
- **Caldera Exhibit 1:** draggable table + five-year water-quality line chart.
- **Caldera Exhibit 2:** draggable programme-threshold table + trajectory/threshold comparison visuals; the exhibit provides criteria but does not pre-compute the year each threshold is achieved.
- **Norvale Exhibit 1:** draggable table + forest-cover recovery line chart.
- **Norvale Exhibit 2:** draggable raw programme-input table + programme-scale area comparison; no calculated duration, programme cost or budget-headroom result is displayed before Analysis.
...
| Salvanova Forest | Exhibit 2 — Vegetation recovery and tourism | Two separate with-wolf trend visuals | Vegetation (%) and tourism ($000) remain on separate scales. The no-intervention Year-5 vegetation assumption is supplied in Study Information, not repeated here. |
...
| Caldera Marsh | Exhibit 2 — Programme thresholds | Three trajectory + threshold-reference mini charts | The table supplies only the success criteria; the visual overlays each criterion on the projection without labelling the first year achieved. |
...
| Norvale Highlands | Exhibit 2 — Programme delivery constraints | Raw programme-scale area comparison | The visual compares only supplied area inputs (wildfire-affected area vs planned active restoration). Capacity, cost, budget and carbon remain raw table inputs for later analysis. |
...
**Evidence-placement rule:** client-supplied scenario assumptions belong in Study Information; exhibits should contain supporting source evidence or programme criteria. No exhibit visual/table may pre-compute a later Analysis answer.
...
- Caldera threshold-reference trajectories and Norvale programme-scale bars follow the global hover/focus/tap exact-value rule; no derived Analysis answers are printed in those visuals.
...
- **Salvanova Forest:** ecosystem intervention / apex-predator restoration. The client-supplied no-intervention Year-5 vegetation assumption (65%) belongs in Study Information; the with-wolf trajectory remains Exhibit 2 evidence.
...
Primary Concern comes after the debrief and synthesises the pattern across all four Task 2 scoring dimensions. The Performance Snapshot shows earned weighted points out of 10 / 30 / 25 / 35 respectively, rather than raw component percentages. There is no separate Assessment Benchmark card.

The learner-facing detailed review includes only Site Profiling and Treatment Construction. Categorisation & Transfer and Prospect Selection remain scored internally and inform the client synthesis but do not render card-by-card or round-by-round logs. Treatment Construction shows only submitted trio, achieved effectiveness, maximum feasible effectiveness and Correct / Needs review.



# V2.2.10 — Red Rock Investigation Evidence Placement & Answer-Leakage Lock

## Global exhibit rule
Investigation exhibits may present **observed data, supplied projections, programme criteria and raw client assumptions**. They must not display a calculated conclusion that a later Analysis question explicitly asks the learner to derive. Study Information holds client-supplied situation assumptions; exhibits hold supporting evidence/criteria; Analysis performs the calculation or judgement.

### Salvanova Forest — Exhibit 2
- The supplied **no-intervention Year-5 vegetation outcome = 65%** belongs in **Study Information** as a Fairhaven planning assumption and remains draggable/click-addable there.
- Exhibit 2 contains the **wolf-reintroduction vegetation trajectory** and **tourism revenue-index trajectory** only.
- Remove any standalone Scenario Reference block, no-wolf comparison row or no-wolf reference mark from Exhibit 2.
- Do not invent intermediate no-wolf vegetation values.
- Analysis Question 4 reads the 65% no-intervention assumption from Study Information and then derives the tourism consequence.

### Caldera Marsh — Exhibit 2
- The threshold table contains only **Indicator** and **Programme threshold**:
  - Nitrogen ≤ 5.0 mg/L
  - Phosphorus ≤ 0.50 mg/L
  - Dissolved oxygen ≥ 5.0 mg/L
- Remove the former **First year met** column and all pre-computed Year 3 / Year 4 / Year 5 threshold-achievement labels.
- The visual may overlay each threshold on the corresponding Exhibit 1 trajectory, but it must not state the first year the criterion is achieved.
- The learner determines the earliest year when the full threshold set is satisfied during Analysis.

### Norvale Highlands — Exhibit 2
- Exhibit 2 contains raw programme inputs only: active restoration area, planting capacity, restoration cost, programme budget and mature carbon benefit.
- The visual compares only raw same-unit scale information: **wildfire-affected area (24,000 ha)** versus **planned active restoration (16,000 ha)**.
- Do not pre-compute or display **5 years planting duration**, **$72m programme cost**, **$3m headroom**, or any other later Analysis result.
- Mixed-unit inputs remain in the table rather than being forced onto one numerical axis.

### Evidence integrity
- Existing source values, units, answer keys, report fields, scoring and downstream mechanics remain unchanged.
- Tables remain the exact-value source for draggable/click-addable Exhibit evidence.
- Charts remain interpretation aids and follow the global hover/focus/tap exact-value rule.
- Where the same raw client fact appears in Study Information and is used to contextualise an exhibit visual, it is not duplicated as a second draggable exhibit fact unless the client file explicitly supplies it in both places.

# V2.2.11 — Red Rock Learner-Facing Scaffolding Reduction

This section is authoritative wherever an earlier section conflicts.

## Analysis question hierarchy
- Keep the overall Analysis page title **Questions**.
- Each of the four Analysis sections keeps only the compact section label **Question 1**, **Question 2**, **Question 3** or **Question 4**.
- Remove the duplicated large section heading that repeats the same `Question N` text immediately beneath the compact label.
- Keep the scenario-specific directions and all answer fields unchanged.
- Apply consistently to Salvanova Forest, Caldera Marsh and Norvale Highlands.

## Red Rock exhibit/header unit treatment
- Remove standalone top-right unit pills/badges from Red Rock exhibit headers and generated Graph Selection preview headers.
- Units remain visible where they carry analytical meaning: table headings/cells, data-entry fields, chart tooltips/accessibility labels and Research Journal evidence values.
- This rule removes redundant UI chrome only; source units and quantitative data remain unchanged.

## Result review — scoring mechanics stay internal
- Remove the learner-facing Graph Selection `Scoring rule` notice from the detailed Result review.
- Graph Selection remains internally **all-or-nothing at 10%**: an accepted chart type and every required chart-data value must all be correct for the component to score 10%; otherwise it scores 0%.
- The detailed review begins directly with **Chart choice** followed by the individual chart-data entries, each using only **Correct** or **Needs review**.
- Do not expose scoring-engine instructions merely to explain how the aggregate percentage was calculated.

## UX principle
Learners see the **client task, evidence, analytical prompts and coaching feedback**. Repeated labels, redundant unit chrome and implementation/scoring instructions stay out of the primary learning interface unless they materially help the decision task.



# V2.2.12 — Sea Wolf Guidance Integrity + Norvale Exhibit 2 Visual Refinement

This section is authoritative wherever earlier learner-facing copy or visual-treatment notes conflict with it. **No Sea Wolf deck, route key, prospect-pool rule, treatment-effectiveness rule, feasible maximum or scoring weight changes in V2.2.12.** The change is to learner-facing guidance integrity and Norvale Exhibit 2 presentation.

## Global Sea Wolf client-guidance rule
Dr. Paula Reyes may explain the **decision objective, information available, client constraints and trade-offs**, but she must not identify the preferred option, hidden decision rule, exact residual answer range, or scoring formula before the learner makes the relevant decision.

The learner-facing distinction is:

**Client evidence and constraints = visible.**  
**How to solve them = learner responsibility.**

Apply this consistently across Cinder Bay, Tidal Reach and Azure Shelf.

### Site Brief
- Keep the full current-site ranges, desired trait, undesired trait and limited Next Site Insight visible.
- Do not instruct the learner to treat Current Site as an automatic first priority or Next Site as a secondary fallback.
- Dr. Reyes should frame the task as making the most defensible programme decision from the current and limited forward-looking information available.

### Characteristics
- Remove any Dr. Reyes statement that names the preferred attribute, preferred range, preferred trait, or preferred attribute+trait pair.
- Use strategy guidance that teaches discrimination without revealing the answer:
...
- Do not teach `farthest from the neutral midpoint` as a deterministic shortcut in the learner-facing prompt.

### Categorisation / routing
- Do not imply a hidden Current Site → Next Site → Return decision hierarchy.
- Learning Mode guidance should ask the learner to compare the microbe with current-site requirements and available next-site insight and choose the most defensible destination.
- After a submitted routing decision, qualitative feedback may explain alignment, conflict or option-preservation logic, but it must not expose hidden counts such as `2 of 4`, `3 of 4`, or the positive-match threshold.

### Initial Pool
- The six-card pool remains deterministically generated from the two selected Characteristics exactly as before.
- Remove feedback that says the learner combined the `most discriminating reference attribute` with the desired trait or otherwise states the hidden preferred profiling formula.
- Feedback should discuss **coverage, gaps and option flexibility** only.

### Prospect Selection
- Keep portfolio-oriented guidance such as checking what the pool is missing and avoiding duplication.
- Do not identify the preferred candidate before the learner confirms a prospect choice.

### Treatment
- Keep the current-site ranges and traits visible.
- When two treatment slots are filled, do not display the exact residual Permeability / Mobility / Energy range required of the third microbe.
- Use qualitative guidance:
  > **With two slots filled, reassess the combination before choosing the third. Think about what the final microbe still needs to contribute across the three attributes and the site traits for the trio to work as a whole.**
- Residual mathematics remains a valid internal/learner-derived strategy; the application simply does not calculate and hand the answer to the learner.

### Treatment Result
- Dr. Reyes may explain which site requirements the submitted trio satisfies or misses after the treatment is locked.
- Do not expose scoring-engine language such as `each missed condition reduces the result by 20 percentage points`.
- The submitted effectiveness and site maximum feasible effectiveness remain visible where already specified by the Result architecture.

## Norvale Highlands — Exhibit 2 UI refinement
The Exhibit 2 visual remains a **raw programme-scale comparison**, not a calculation aid.

- Compare only:
  - **Wildfire-affected area — 24,000 ha**
  - **Planned active restoration — 16,000 ha**
- Use a common horizontal **0–24,000 ha scale**.
- Place each category label and its exact value clearly above its bar.
- Use slim, visually equivalent bars with no heavy selected-state outline.
- Keep exact values permanently visible and preserve hover / keyboard focus / tap access to the same values under the global chart-interaction rule.
- Use compact tooltip labels so the tooltip does not obscure the comparison.
- Remove assessment-like copy such as `Cost, duration and budget headroom remain for the consultant to calculate in Analysis.`
- Use neutral exhibit guidance:
  > **Compare the scale of wildfire impact with the area proposed for active restoration.**
- The structured table underneath remains the exact-value evidence source and Research Journal interaction surface.
- Do not display calculated planting duration, programme cost, budget headroom or any later Analysis answer.

## Global SVG bar-hover correction
Hover/focus styling for bar charts must apply the outline to the **bar mark itself**, not the enclosing SVG group. Text labels must never acquire a dark stroke or appear duplicated when a bar is hovered or keyboard-focused.

## V2.2.12 preservation lock
- Red Rock source values, units, Analysis answers, Written Report fields, Graph Selection scoring and Visual Report cases remain unchanged.
- Sea Wolf numeric data, traits, categorisation decks, initial-pool generation, prospect rounds, treatment logic, feasible maxima and four-dimension Task 2 scoring remain unchanged.
- V2.2.12 does not implement or alter any pre-Site-1 routing mechanic; the existing Sea Wolf task sequence remains the V2.2.11/V2.2.8 sequence unless changed by a later explicit specification.


# V2.2.13 — Locked-routing feedback alignment

This section is authoritative wherever earlier Sea Wolf Learning Mode routing-feedback copy conflicts with it. **No routing mechanic, reference route, site data, scoring weight, transfer rule, prospect rule or treatment rule changes in V2.2.13.** The change is limited to feedback after a routing choice has already been committed.

## Post-routing feedback rule

A submitted Categorisation or Transfer Review allocation is locked before Dr. Paula Reyes responds. Therefore the feedback must not imply that the learner can revise the just-submitted card.

- Remove phrases such as `I would challenge that route`, `Recheck what I need at this site`, `choose between...`, or any equivalent instruction to redo the committed allocation.
- Do not reveal the reference destination after an incorrect committed route.
- Do not expose the hidden routing threshold or positive-match counts.
- Feedback should acknowledge the quality of the committed judgement and redirect attention to the **remaining site decisions**.

### Correct / reference-aligned route

Use neutral reinforcement:

> **That is a defensible route. Keep using the current site requirements and the limited next-site information as you work through the remaining site decisions.**

On the final site, omit the unavailable next-site reference:

> **That is a defensible route. Keep using the current site requirements as you work through the remaining site decisions.**

### Non-reference route

Use non-reversible coaching:

> **This may not be the strongest route. Keep the current site requirements and the limited next-site information in mind as you work through the remaining site decisions.**

On the final site:

> **This may not be the strongest route. Keep the current site requirements in mind as you work through the remaining site decisions.**

## Global Sea Wolf feedback principle

**Once a routing decision is locked, feedback reflects on the submitted judgement and supports future decisions; it never instructs the learner to revise a choice that the interface no longer allows them to change.**


# V2.2.14 — Streamlined Sea Wolf Result Review

This section is authoritative wherever earlier Result-page or Sea Wolf site-result notes conflict with it. **Task 2 data, scoring weights, route keys, profiling logic, prospect logic, treatment logic and feasible maxima are unchanged.**

- Remove the separate site-result `Reference maximum feasible effectiveness` note.
- Keep Dr. Reyes site-result feedback concise and client-facing; do not repeat the displayed percentage or expose detailed scoring-engine arithmetic in the statement.
- Remove `Selected attempt` and the simulator disclaimer from the Results hero.
- Remove the repeated Sea Wolf study title from the Workstream 02 completion strip.
- Performance Snapshot shows earned weighted points out of 10, 30, 25 and 35.
- Site Profiling detailed review retains only selected lens, reference lens and status.
- Remove learner-facing Detailed Review sections for Categorisation & Transfer and Prospect Selection.
- Treatment Construction detailed review retains only submitted treatment, achieved effectiveness, maximum feasible effectiveness and status.
- Remove `Performance against feasible maximum`, trio-average/target strings and constrained-site explanatory paragraphs from the detailed review.
- Remove the generic Reasoning Profile, Primary Performance Concern and Recommended Next Practice sections from Results.
- Readiness Confidence remains, but without the generic consistency instruction; Return Home is the only closing action.
