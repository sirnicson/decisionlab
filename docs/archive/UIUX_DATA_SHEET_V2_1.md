# Solve It Readiness Lab — UI/UX DATA SHEET V2.1
Version: 2.1 — Integrated Engagement Home + Contextual Evidence Lock
...
- meaningful evidence phrases remain in their sentence position and are directly draggable or click-addable to the Research Journal;
...
Investigation exhibits are presented as compact analytical tables built from the authored row/column or label/series data.

Examples for Salvanova:
- Exhibit 1: rows for Years 0–5 with separate No Wolves and With Wolves columns;
- Exhibit 2: rows for Years 0–5 with vegetation-cover and tourism columns.

Each numeric or numeric-bearing data cell is individually draggable or click-addable while remaining in its row/column location. Chart rendering is retained for later Graph Selection and Visual Report contexts where a chart is the analytical object.
...
- Study Information evidence must remain readable as normal prose rather than being converted into input boxes;
- Exhibit values remain in structured rows/columns rather than repeated as standalone cards;
- the Journal keeps the evidence label, value/unit and source location (`Study Information`, `Exhibit 1`, or `Exhibit 2`);
...
## Mode icons
- Learning Mode uses the supplied single-person icon to signal guided work with client input.
- Simulation Mode uses the supplied three-person/team icon to signal independent work under assessment conditions.
...
- Visual Report utility layout for all Red Rock studies: **left = stage navigation, centre = active case, right = calculator**. The Research Journal is not shown in Visual Report.
- Analysis utility layout remains **calculator in the left rail + Research Journal on the right**.
