# Solve It Readiness Lab — UI/UX DATA SHEET V1.2.2
Version: 1.2.2 — Consistency, Gameability + Client-Narrative UI Review Lock
Status: SOURCE OF TRUTH — CONSULTING NARRATIVE, LOCKED TASK 1/TASK 2 MECHANICS, OPERATIONAL SEA WOLF DECKS, RESEARCH-INFORMED DATA GOVERNANCE, AND VISUAL SYSTEM LOCKED
...
- Simulation Mode continues to hide site-effectiveness detail until the final Results/Review.
...
- In a full simulation, the final Results screen presents these sponsor views as a short **client close-out meeting** before the existing scorecards, reasoning profile and decision review.
...
- keep Prospect Selection editable until confirmation, with decision quality assessed later in Results/Review rather than revealed on card selection;
...
2. **Analysis** — one long page containing Question 1–4; Calculator in the left rail; Research Journal full-height right
3. **Report** — Written Report → Graph Selection → Visual Report
4. **Visual Report** — Case 1 → Case 2 → Case 3 → Case 4 → Case 5 → Case 6
5. **Task 1 Result / Review**
...
In Simulation Mode, users should not freely jump between stages once a lock point has been passed.
...
- top status bar;
- left navigation/context rail where useful;
- central task workspace;
- right utility panel for Journal / Site Information / Notes.
...
> **Authoritative V1 workflow.** This section supersedes earlier Red Rock references that described the six cases as a separate Visual Report cases stage. In V1, the six cases are part of **Report → Visual Report**.
...
## 7.1 Three-stage Red Rock architecture
Red Rock contains only three primary stages:

`INVESTIGATION → ANALYSIS → REPORT`

### INVESTIGATION
Expandable/clickable left-navigation items:
- Objective
- Study Information
- Exhibit 1
- Exhibit 2
...
### ANALYSIS
Expandable left-navigation items:
- Questions
- Question 1
- Question 2
- Question 3
...
All four question sections appear on **one vertically scrollable Analysis page**. Clicking a question label scrolls to the relevant section.
...
When Visual Report is active, it expands further to:
- Case 1
- Case 2
- Case 3
- Case 4
...
There is no standalone Visual Report cases stage in V1.

## 7.2 Investigation — evidence environment
...
- plausible non-essential observations;
- enough information to require evidence selection.

### Exhibit 1 and Exhibit 2
Each is a substantive table or chart with multiple data points relevant to later Analysis/Report work.

Examples for Salvanova:
- Exhibit 1: projected hare population with and without wolves across Years 0–5;
- Exhibit 2: vegetation recovery and tourism values over time.

### Research Journal
Research Journal is visible on the right throughout Investigation.

Primary interaction:
`source evidence → drag/move → Research Journal`

Rules:
- do not use a large `Add to Journal` button as the main interaction;
- every displayed numerical chart/table data point that may be useful should be draggable or otherwise movable to the Journal;
- the Journal keeps the source label and value;
- collected Investigation evidence may be returned before the Analysis lock;
- keyboard-accessible move/return equivalents must remain available.

## 7.3 Analysis — one page, four question sections
The Analysis stage uses one scrollable working page containing Question 1–4.

Each question section may contain one or more input fields.

### Analysis tool placement
Analysis displays both tools, but on different rails:
- **Left rail:** Onscreen Calculator beneath the active stage/subpage navigation.
- **Right rail:** Research Journal occupying the full usable rail height.

The Calculator must never share the right rail with the Research Journal.

### Calculator interaction
The calculator is functional, not decorative.

Supported workflow:
`Research Journal value → calculator → calculation → calculator result → answer field`

- Journal numerical values can be dragged into the calculator input;
- calculator supports basic arithmetic and retains short calculation history;
...
When an Analysis answer is populated, a derived Journal item is created/updated, for example:

`Analysis output — Hares consumed per year: 75,000`

These derived values remain available for later Analysis questions and the Report stage.

### Learning vs Simulation
Learning Mode may show reasoning prompts. Simulation Mode does not show correctness, expected answers or hints before submission.

## 7.4 Analysis completion and lock
At the bottom of the Analysis page, display `Complete Analysis`.

Selecting it opens a confirmation modal:
...
- Investigation is locked;
- Analysis is locked;
- source exhibits are no longer directly accessible;
- Research Journal persists;
- Report → Written Report opens.

## 7.5 Report — Written Report
Written Report is a single report page with narrative text containing input fields.

The learner can:
- drag values from the Research Journal into report fields;
- type values directly.

**Calculator is not displayed on Written Report.**

At the bottom, `Complete Written Report` opens a review/confirmation warning. After confirmation the Written Report locks and Graph Selection opens.

## 7.6 Report — Graph Selection
Graph Selection contains:
1. directions/question;
2. three chart-style options;
3. chart-data input fields after a chart is selected;
4. a generated chart preview that updates from entered data.

Inputs may be populated by:
- drag from Research Journal;
- keyboard entry.

**Calculator is not displayed on Graph Selection.**

Research Journal remains visible.

At the bottom, `Complete Graph Selection` opens a review/confirmation warning. Confirmation locks the graph task and opens Visual Report.

## 7.7 Report — Visual Report Cases 1–6
Visual Report contains six individual case pages.

Every case contains:
- Directions / background context;
- a substantive chart or table;
- the question below the exhibit;
- answer fields;
- an onscreen calculator on the right.

**Research Journal is not displayed during Visual Report Cases.**

The six cases remain independent and may test:
- ratios and carrying capacity;
- percentage-point change;
- constrained optimisation;
- ranges/stability;
- cost effectiveness;
- fixed-budget allocation;
- related quantitative/comparative reasoning.

Case navigation is shown under `Visual Report` in the left rail.

At the end of Case 6, `Complete Visual Report` opens a final review/confirmation modal. Confirmation completes both Visual Report and Task 1 Red Rock.

## 7.8 Tool-availability matrix

| Red Rock substage | Research Journal | Onscreen Calculator |
|---|---:|---:|
...
## 7.9 Task 1 result before Task 2
After Task 1 completion, show a **Task 1 result for the selected Red Rock case only**.

It must not aggregate other Red Rock studies.
...
In Simulation Mode the primary continuation action is:
...
I am reviewing whether Fairhaven should support a wolf-reintroduction programme in Salvanova Forest. The loss of the apex predator has changed the ecosystem trajectory, and I need to know whether intervention is justified and what ecological and economic consequences I should expect.
...
**Sea Wolf complete → Task 2 Results/Review.**
...
- principal performance constraint;
...
| Evidence selection / Research Journal efficiency | 5% |
| Time discipline | 5% |
| **Total** | **100%** |
...
### Visual report
Score:
- appropriate chart type for the analytical question;
- correct values;
- correct comparison structure;
- accessible and non-misleading representation.
...
### Evidence selection / Journal efficiency
Reward:
- collection of decision-relevant facts;
- avoidance of excessive irrelevant information;
- effective organisation of useful information.

Do not heavily penalise exploratory collection. This metric should distinguish disciplined evidence selection from indiscriminate collection rather than reward minimal clicking.

### Time discipline
Use the 35-minute Task 1 allowance.

Time should be a **small scoring component**, not a substitute for accuracy.

Finishing early provides no bonus if answer quality is lower.

---

## 11.4 Task 2 — Sea Wolf score

The selected Sea Wolf study contains exactly **three sites**.

The Task 2 score must evaluate both:
1. the quality of the final treatment outcomes; and
2. the quality of the decisions that produced them.

### Weighting

| Component | Weight |
|---|---:|
| Treatment effectiveness across 3 sites | 45% |
| Categorisation allocation quality | 20% |
| Prospect-selection quality | 20% |
| Cross-site portfolio preservation | 10% |
| Time discipline | 5% |
| **Total** | **100%** |

### Treatment-effectiveness component

For each site, calculate the submitted treatment using the fixed treatment formula:

Start at 100% and deduct 20 percentage points for each failed condition:
1. average Permeability outside target;
2. average Mobility outside target;
3. average Energy outside target;
4. no desired trait;
5. one or more undesired traits.

### Maximum-feasible normalisation

Some constrained sites may deliberately have no feasible 100% solution.

Therefore, do **not** penalise the learner merely because the site maximum is below 100%.
...
If Azure Shelf Site 3 has:
- maximum feasible effectiveness = 80%;
- submitted effectiveness = 80%;

then:

`80 / 80 × 100 = 100`

The learner has therefore achieved the **best feasible treatment** and receives full treatment-performance credit for that site.

This prevents constrained cases from being structurally harder to score well on simply because their optimum is intentionally imperfect.

### Categorisation allocation quality

Each microbe allocation is evaluated against the fixed case model.

Use judgement classes:
- **Strong allocation**
- **Defensible but costly later**
- **Weak allocation**

Suggested V1 numeric mapping for scoring:
- Strong allocation = 100
- Defensible but costly later = 65
- Weak allocation = 20

The UI does not reveal these numeric values during Simulation Mode.

### Prospect-selection quality

Score each of the four 1-of-3 prospect rounds using the same decision-quality principle.

Early Cinder Bay rounds should usually contain one clearly strong choice.

Later Tidal Reach and Azure Shelf rounds may contain two defensible options, but one may preserve materially better **final-treatment possibilities within the current site's pool**.

### Cross-site portfolio preservation

This component measures whether **routing decisions** that appear attractive for the current site unnecessarily damage later-site options.

Examples:
- preserving a scarce desired-trait microbe for a later site;
- avoiding use of a current-fit microbe that is uniquely valuable downstream;
- accepting a slightly weaker local candidate when it preserves a stronger total three-site outcome.

This is especially important in Azure Shelf.

### Outcome-based solution rule

The system must **not require one memorised canonical trio**.

If multiple three-microbe treatments produce the same maximum feasible result, all receive equivalent final-treatment credit.

The canonical solution stored in test data is a reference path, not the only accepted answer.
...
- Written report: 88%
- Visual report: 100%
- Visual Report Cases: 5 / 6
- Evidence selection: Strong
- Time used: 32:18 / 35:00

Then show:

**Primary loss**
`One constraint error in Visual Report Case 4`

The exact diagnostic should be derived from the recorded error taxonomy.

---

# 11.11 Task 2 result card
...
Display:
- Site 1 treatment: 100% of feasible optimum
- Site 2 treatment: 100% of feasible optimum
- Site 3 treatment: 80% submitted / 80% maximum feasible = 100% of feasible optimum
- Categorisation quality: 84%
- Prospect-selection quality: 79%
- Cross-site portfolio preservation: 76%
- Time used: 28:42 / 30:00

For an intentionally imperfect site:

`Site 3 treatment: 80% submitted / 80% maximum feasible = 100% of feasible optimum`

This makes clear that an 80% treatment may represent perfect decision quality when 80% is the mathematical maximum.
...
# 11.13 Primary Performance Constraint
...
The final screen should appear in this order:
...
5. **Task 2 result**
6. **Reasoning Profile**
7. **Primary Performance Constraint**
8. **Recommended Next Practice**
9. **Attempt history / readiness confidence**
10. **Review Decisions**
...
# 11.17 Review Decisions screen

From the result pane, the user can open a detailed replay.
...
- decisions classified as Strong / Defensible but costly later / Weak;
- downstream consequence of key allocations.

This review is available after completion and must not leak answers during Simulation Mode.
...
- scalable text;
- chart values also represented in accessible table form.
...
- [ ] Red Rock has working Investigation (Objective / Study Information / Exhibit 1 / Exhibit 2), Analysis (4 scrollable question sections + Calculator + Journal), Report (Written / Graph / Visual), and 6 Visual Report case pages.
...
I am reviewing a five-year restoration proposal for Caldera Marsh after several years of elevated agricultural runoff. Water quality has declined, and I need to know whether the programme is likely to restore the marsh, what could limit delivery, and what ecological and financial consequences I should expect.

**Your assignment**

I need you to assess the expected impact on nutrient concentrations, dissolved oxygen, fish populations and local visitor revenue, then turn that evidence into a clear recommendation. Use the Research Journal and available analytical tools to keep the evidence traceable; in a full simulation, complete the workstream within 35 minutes.
...
- two observation towers built 11 years ago;
- 16% of visitors arrive by bicycle.

Purpose: force the learner to decide what matters.
...
I am reviewing a five-year recovery programme for Norvale Highlands after a major wildfire damaged forest cover, habitat and watershed resilience. I need to know whether the proposed plan is operationally feasible as well as environmentally attractive.

**Your assignment**

I need you to test whether the programme can meet its land-recovery, carbon and watershed-resilience targets within the available budget and planting capacity, then give me a decision-ready recommendation. Use the Research Journal and analytical tools provided; in a full simulation, complete the workstream within 35 minutes.
...
Distractors:
- visitor centre rebuilt in 2019;
- three hiking routes;
- ranger fleet age;
- annual snowfall.
...
I want you to work site by site. Choose two characteristics to shape the six-microbe treatment pool; separately route ten microbes between the current site, the next site where available, or return; then add one candidate from each of four prospect sets and recommend a final three-microbe treatment. I will keep the full current-site requirements and limited next-site information visible as your working reference. Once you confirm a treatment, that site is closed; in a full simulation, I expect all three sites completed within 30 minutes.
...
Use:
`stage rail | central analytical workspace | Research Journal`

- workspace background: off-white;
- exhibits and questions: white cards;
- Research Journal: white utility surface or dark-green utility treatment where useful;
- active stage / selected facts / progress: darker lime;
- tables: white with subtle separators, minimal grid lines;
- numeric content must remain highly legible.

## 22.16 Sea Wolf workspace styling

Use:
`stage/site rail | microbe interaction workspace | Site Information / Notes`
...
This screen should use the visual reference most strongly.

Top:
- large darker-lime feature panel;
- left: readiness title, result band, simulation pair;
- right: large score ring.

Below:
- Task 1 score;
- Task 2 score;
- primary focus / constraint cards;
- Reasoning Profile;
- Primary Performance Constraint;
- Recommended Next Practice;
- Decision Review action.

Avoid leading with a dense table.

## 22.18 Responsive design

### >=1200px
- persistent `288px` sidebar;
- full three-column task workspaces where applicable;
- wide hero composition.

### 768–1199px
- reduced shell width / collapsible navigation;
- smaller page gutters;
- utility panes may stack or convert to drawers.
...
## Red Rock panel arrangement refinement

### Embedded guidance placement
- Do not place a Reasoning Flow component in the left rail.
- Short Dr. Cross/client prompts may appear naturally on the Investigation Objective page and at relevant decision points in Learning Mode.
- The left rail remains reserved for stage/subpage navigation and Calculator use.

### Calculator placement
- During **Analysis** and **Visual Report**, the onscreen Calculator appears in the **left rail beneath the active stage/subpage navigation**.
- The Calculator does not share the right rail with the Research Journal.
- During Written Report and Graph Selection, the Calculator remains hidden.

### Research Journal placement
- Where the Research Journal is available, it occupies the **full usable height of the right rail**.
- The right rail is dedicated to the Journal so collected and derived evidence remain continuously visible while working.
- During Visual Report, the Research Journal remains unavailable as previously specified.
...
### Written Report density — V1 locked requirement

The Red Rock Written Report must mirror the information density of the reference assessment rather than functioning as a short summary sentence.

For every Red Rock study:
- use **at least two coherent paragraphs**;
...
- when completed, the filled text should read as a concise analytical report, not a sequence of disguised questions.

Recommended structure:
- **Paragraph 1:** baseline / no-intervention / problem trajectory;
- **Paragraph 2:** intervention / programme outcome / ecological, operational, or economic consequence.

The exact content varies by case, but Salvanova, Caldera Marsh and Norvale Highlands must maintain comparable report density and synthesis difficulty.

---


# APPENDIX 0 — V1.2 CONSISTENCY RESOLUTIONS

This appendix records the implementation choices that supersede conflicting legacy text elsewhere in earlier drafts.

| Topic | V1.2 authoritative rule |
|---|---|
| Learner-facing reasoning | No `Reasoning Flow` component. Use client/context prompts and post-task decision review. |
| Red Rock Analysis layout | Calculator in left rail; Research Journal full-height in right rail. |
...
| Sea Wolf routing vs treatment | Separate decks and separate functions; routing never creates the Initial Prospect Pool. |
| Sea Wolf operational data | Every site has routing 10 + initial 6 + four 3-card prospect rounds + final 3 treatment validation. |
| Prospect vs cross-site reasoning | Prospect Selection optimises the current treatment pool; cross-site opportunity cost is measured through routing/transfers. |
| Scientific data boundary | Numeric game values are synthetic; research supports only relationships, traits, sensitivities and trade-offs. |
