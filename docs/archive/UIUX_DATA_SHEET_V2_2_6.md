# Solve It Readiness Lab — UI/UX DATA SHEET V2.2.6
Version: 2.2.6 — Consolidated Navigation, Workspace, Integrated Results Review, Global Chart Data-Point Visibility, Contextual Graph Selection, Written Report Integrity & Integrated Client Benchmark
...
2. **Primary Concern** — a client-facing synthesis that internally evaluates all five weighted Red Rock dimensions together rather than displaying a separate Assessment Benchmark scorecard;
...
The five weighted component scores remain part of the scoring engine, but they are interpreted inside Dr. Cross's Primary Concern narrative. The learner does **not** see a detached Assessment Benchmark panel or a separate component score table on the Task 1 Result page.
...
| Evidence selection / Research Journal efficiency | 10% |
| **Total** | **100%** |
...
The Red Rock scoring engine retains exactly five weighted dimensions:
- Analysis accuracy — **40% of Task 1**
- Written Report — **20% of Task 1**
- Graph Selection — **10% of Task 1**
- Visual Report Cases — **20% of Task 1**
- Evidence Selection — **10% of Task 1**

These weights total **100%**, but the Task 1 Result page does **not** show them as a detached Assessment Benchmark panel. Instead, the five component outcomes are synthesised into **Primary Concern**, written as Dr. Cross's client assessment of what would or would not prevent the recommendation from being taken forward.
...
The threshold wording is component-specific for Analysis, Written Report, Graph Selection, Visual Report Cases and Evidence Selection. The Primary Concern must consider **the pattern across all five dimensions**, including how evidence supports analysis, how analysis carries into the written report, and whether the visual communication and case judgments are consistent with the technical work. It must not simply name the lowest raw score or the largest weighted gap.
...
- sound Analysis/Report + weak Evidence Selection → the recommendation is stronger than the evidence trail supporting it;
...
- all five components at client-ready level → no material concern prevents sign-off.

Then show the integrated detailed review immediately on the same Result page:
- Dr. Cross client debrief;
- **Primary Concern** with the five-dimension benchmark interpretation embedded in the narrative;
...
- Evidence Selection review showing whether required decision-relevant evidence was retained and whether low-value distractor evidence was unnecessarily kept.
...
# 11.11 Task 2 Result — client feedback, assessment and integrated explanation
...
The same Task 2 Result page also contains the integrated detailed review:
- Dr. Paula Reyes client debrief;
- site targets and routing decisions;
- prospect-selection choices and pool consequences;
- submitted treatment, averages and trait checks;
- raw effectiveness and maximum feasible effectiveness;
- strengths, weaknesses, alternative high-quality choices where useful, and concise reasoning explanation.

There is no separate Decision Review destination.
...
- **Salvanova Exhibit 2:** draggable table + vegetation/tourism bar chart.
...
| Salvanova Forest | Exhibit 2 — Vegetation recovery and tourism | Two separate mini trend charts in one visual panel | Vegetation (%) and tourism ($000) have different units, so they must not share one y-axis. |
...
- Red Rock weighting = Analysis 40 + Written Report 20 + Graph Selection 10 + Visual Report Cases 20 + Evidence Selection 10 = **100%**;
...
# V2.2.6 — RESULT REVIEW SIMPLIFICATION AND INTEGRATED CLIENT BENCHMARK
...
- integrate the five weighted Red Rock component outcomes into the **Primary Concern** client narrative;
- Primary Concern evaluates the performance pattern across Analysis, Written Report, Graph Selection, Visual Report Cases and Evidence Selection rather than simply selecting the lowest score;
...
- detailed-review panel headers are only **Detailed review · Analysis**, **Detailed review · Written Report**, **Detailed review · Graph Selection**, **Detailed review · Visual Report**, and **Detailed review · Evidence Selection**;
...
- Evidence Selection detailed review reports required evidence retained and unnecessary low-value evidence retained so the client synthesis remains traceable to the underlying submission.
